...
public class ConsumableItem extends Item {
  ...
  private void triggerConsumptionEvent(Level level, LivingEntity entity) {
    var event = (this.type == Type.ONLY_DRINK) ? GameEvent.DRINK : GameEvent.EAT;
    level.gameEvent(entity, event, entity.getEyePosition());
  }
  ...
  public void appendHoverText(@NotNull ItemStack itemStack, @Nullable Level level,
    ...
    if (this.water > 0) {
      tooltip.add(Component.translatable("item.craftingdeadsurvival.consumable.water_info").withStyle(
          ChatFormatting.GRAY).append(Component.literal(" " + this.water).withStyle(ChatFormatting.RED)));
    }
  }
}
