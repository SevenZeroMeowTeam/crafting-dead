...
public class JuggernautZombieHandler extends ZombieHandler {
  ...
  protected ItemStack createHeldItem() {
    ...
    var gunStack = ModItems.M240B.get().getDefaultInstance();
    var gun = gunStack.getCapability(Gun.CAPABILITY);
if (gun != null) {

      var magazineStack = ModItems.M240B_MAGAZINE.get().getDefaultInstance();
  ...
  protected ItemStack createHatItem() {
    return ModItems.BLACK_BALLISTIC_HAT.get().getDefaultInstance();
  }

  @Override
  protected ItemStack createClothingItem() {
    return ModItems.JUGGERNAUT_CLOTHING.get().getDefaultInstance();
  }

  @Override
  protected ItemStack createVestItem() {
    return this.createFilledVestItem(
        CraftingDeadSurvival.serverConfig.juggernautZombieVestEquipChance.get().floatValue(),
        ModItems.RIOT_VEST.get().getDefaultInstance(),
        this.createVestLootId("juggernaut_zombie_vest_loot"));
  }
  ...
  public void applyEquipmentDropChances() {
    ...
    extension.setEquipmentDropChance(Equipment.Slot.CLOTHING,
        CraftingDeadSurvival.serverConfig.juggernautZombieClothingDropChance.get().floatValue());
    extension.setEquipmentDropChance(Equipment.Slot.HAT,
        CraftingDeadSurvival.serverConfig.juggernautZombieHatDropChance.get().floatValue());
    extension.setEquipmentDropChance(Equipment.Slot.VEST,
        CraftingDeadSurvival.serverConfig.juggernautZombieVestDropChance.get().floatValue());
    extension.setEquipmentDropChance(Equipment.Slot.BACKPACK,
        CraftingDeadSurvival.serverConfig.juggernautZombieBackpackDropChance.get().floatValue());
    zombie.setDropChance(EquipmentSlot.MAINHAND,
        CraftingDeadSurvival.serverConfig.juggernautZombieHandDropChance.get().floatValue());
