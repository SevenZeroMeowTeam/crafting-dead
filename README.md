...
| `kotlin-refactor-1.20.x` | 1.20.1 | 47.4.22 | 17 | 🔬 Kotlin + Java 混编重构（1.20.x） |
| `kotlin-refactor-1.19.x` | 1.19.2 | 43.5.2 | 17 | 🔬 Kotlin + Java 混编重构（1.19.x） |
...
### v1.9.9 / v1.2.11 / v1.0.11 / v0.0.11（装备/合成界面人物模型位置修复）

**关键修复：装备 / 合成界面人物 3D 模型位置过低**

- 症状：打开装备 / 合成界面时，中间的人物模型位置过低，模型的腿 / 下半身与下方玩家背包格（`y=84` 起）重叠
- 根因：`EquipmentScreen` / `CraftingScreen` 的 `renderPlayerEntity` 把模型中心 Y 设为 `topPos + 72`，
  低于模型窗口（左右两侧分别是武器列与装备列，垂直方向从顶部到背包格），导致模型与背包格重叠
- 修复：模型中心 Y 改为 `topPos + 45`，使其居中于模型窗口，不再与背包格重叠
- 涉及：`EquipmentScreen.java`、`CraftingScreen.java`（改用 `InventoryScreen.renderEntityInInventoryFollowsMouse` 后的坐标修正）
...
> 📌 本版本修复已同步至全部分支：
> - **1.20.x**：`v1.9.7-1.20.1 / v1.2.8-1.20.1 / v1.0.9-1.20.1`
> - **1.19.x**：`v1.9.7-1.19.2 / v1.2.8-1.19.2 / v1.0.9-1.19.2`
> - **kotlin-refactor**：本分支同时进行 Kotlin + Java 混编重构（断肢系统、品质系统、核心枚举与数据类已迁移 Kotlin）
