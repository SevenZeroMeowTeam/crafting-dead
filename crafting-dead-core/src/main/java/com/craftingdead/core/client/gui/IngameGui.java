...
public class IngameGui {
  ...
  public void renderFlashBangOverlay(Player player, GuiGraphics guiGraphics, int width, int height,
      float partialTick) {
  ...
  private void renderKillFeedEntry(KillFeedEntry entry, GuiGraphics guiGraphics,
    ...
    if (!entry.getWeaponStack().isEmpty()) {
      poseStack.pushPose();
      {
        ...
        RenderUtil.renderGuiItem(poseStack, entry.getWeaponStack(),
            0xFFFFFF + (opacity << 24));
      }
      poseStack.popPose();
    }
  ...
  private void renderCombatMode(PlayerExtension<AbstractClientPlayer> player,
    ...
    RenderUtil.fill(poseStack, 0, height - healthBoxHeight, healthWidth, height, 0x55000000);
