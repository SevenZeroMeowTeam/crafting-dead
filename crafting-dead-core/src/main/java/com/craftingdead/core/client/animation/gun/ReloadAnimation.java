...
public class ReloadAnimation extends TimedAnimation {
  ...
  private void applyTranslations(float partialTicks, PoseStack poseStack) {
    ...
    poseStack.mulPose(Axis.YP.rotationDegrees(sineProgress * 30.0F));
    poseStack.mulPose(Axis.XP.rotationDegrees(sineProgress * -30.0F));
    poseStack.translate(0.0D, 0.0D, sineProgress / 4.0D);

    applyRandomMovements(MUTABLE_VEC, partialTicks);
    poseStack.mulPose(Axis.XP.rotationDegrees(MUTABLE_VEC.x));
    poseStack.mulPose(Axis.ZP.rotationDegrees(MUTABLE_VEC.z));
  }
